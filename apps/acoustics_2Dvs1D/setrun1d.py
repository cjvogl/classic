...
import os
import numpy as np
from setrun import setCommonParameters
...
#------------------------------
    ...
    # First set parameters common to both 1d and 2d runs
    setCommonParameters(clawdata,probdata)
    ...
    # Compute domain size & cell # from time & speed info
    if clawdata.output_style == 1:
        tfinal = clawdata.tfinal
    elif clawdata.output_style == 2:
        tfinal = max(clawdata.output_times)
    elif clawdata.output_style == 3:
        print 'Code not set for using output_style 3'
        raise Exception('output_style')
    wspeed = np.sqrt(probdata.bulk/probdata.rho)
    maxdx = 0.0005

    # Lower and upper edge of computational domain:
    clawdata.lower[0] = 0.0          # xlower
    clawdata.upper[0] = 1.0 + tfinal*wspeed  # xupper

    # Number of grid cells:
    clawdata.num_cells[0] = int(np.ceil(clawdata.upper[0]/maxdx))  # mx
    ...
    # Note output parameters set in setCommonParameters
